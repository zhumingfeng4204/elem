        ...
        <ul>
          <li v-for="(item,index) in recommend.items" :key="index">
            ...
            <div class="recommend-food">
              ...
              <p class="recommend-food-zm">月售{{item.month_seles}}好评率{{item.satisfy_rate}}</p>
            </div>
            <div class="recommend-food-price">
              <p>￥{{item.activity.fixed_price}}</p>
              <jiajian :food="item"></jiajian>
            </div>
          </li>
        </ul>
      </div>
    </div>
    <div class="menuview" ref="menuscroll">
      ...
      <div class="foods-wrapper" ref="menuscrollfood">
        <ul>
...
export default {
  ...
  methods: {
    ...
    initscroll() {
      this.menuscroll = new bscoll(this.$refs.menuscroll, {
        click: true
      });
    ...
    getdata() {
      this.$axios("/api/profile/batch_shop").then(res => {
        res.data.recommend.forEach(recommend => {
          recommend.items.forEach(element => {
            element.count = 0;
          });
        });
