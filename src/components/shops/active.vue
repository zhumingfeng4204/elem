<template>
  <div class="rst-activity">
    <div class="activity-txt">
      <span :style="{background: '#'+ activties[0].icon_color}">{{activties[0].icon_name}}</span>
      <span>{{activties[0].description}}</span>
    </div>
    <div class="activity-count" @click="showsheet=true">
      <span>{{activties.length}}优惠</span>
    ...
    <transition name="fade">
      <div class="act-model" v-show="showsheet">
        <div class="activity-sheet">
          ...
          <h2>优惠活动</h2>
          <ul>
            <li v-for="(item,index) in activties" :key="index">
